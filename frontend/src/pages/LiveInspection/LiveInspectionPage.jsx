...
export default function LiveInspectionPage() {
  ...
  const [opacity, setOpacity] = useState(60); // Grad-CAM transparency (0-100)
  const [yoloThreshold, setYoloThreshold] = useState(65);
      ...
      <main className="flex-1 p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-accent/10 pb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button variant="secondary" className="flex items-center gap-1.5 py-1.5 px-3" onClick={handleManualRerun}>
        ...
        {activeBoard && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            {/* LEFT COLUMN: Camera Stream, Heatmap overlays & Explainability */}
            <div className="lg:col-span-8 space-y-6">
              
              {/* Image Feed Card */}
              <GlassCard className="flex flex-col h-[480px]" hoverLift={false}>
                ...
                <div className="flex items-center justify-between border-b border-accent/5 pb-3">
                  ...
                  {/* Mode Toggles */}
                  <div className="flex bg-[#050816] rounded-md p-1 border border-accent/15">
                    {[
                ...
                {/* Viewport Frame */}
                <div className="relative flex-1 bg-black rounded-lg overflow-hidden border border-accent/5 my-4 flex items-center justify-center">
                ...
                {/* Opacity slider bar (Only shown in Grad-CAM mode) */}
                {visualMode === "gradcam" && (
                  <div className="flex items-center gap-4 px-2 py-1.5 border border-accent/10 rounded-lg bg-[#050816]/60">
                    <span className="font-mono text-[9px] text-[#9ca3af] uppercase tracking-wider font-bold flex items-center gap-1">
                    ...
                    <input
                      ...
                      className="flex-1 accent-accent h-1 bg-slate-900 rounded-lg appearance-none cursor-pointer"
                    />
                    <span className="font-mono text-[9px] text-white font-bold w-8 text-right">{opacity}%</span>
                  </div>
                )}
              </GlassCard>
              ...
              <GlassCard className="space-y-3" hoverLift={false}>
                ...
                <p className="font-sans text-xs text-slate-300 leading-relaxed text-left">
                  {activeBoard.gradCamExplanation}
                </p>
                <div className="flex items-center gap-4 text-[9px] font-mono text-slate-500">
                  <span>Target Class Layer: <strong className="text-slate-400">YOLO_head/cv3_d1</strong></span>
                  <span>Feature Extraction: <strong className="text-slate-400">Backbone/SPPF</strong></span>
                </div>
              </GlassCard>

            </div>

            {/* RIGHT COLUMN: X-MCCV Constraints Engine */}
            <div className="lg:col-span-4 space-y-6">
              
              {/* X-MCCV Page details card */}
              <GlassCard className="space-y-5" hoverLift={false}>
                <div className="flex items-center justify-between border-b border-accent/5 pb-3">
                  ...
                  <span className="font-mono text-[9px] text-slate-500 uppercase tracking-widest">Active rules</span>
                </div>
                ...
                <div className="space-y-4 text-left">
                  ...
                  <div className="p-3 bg-[#050816]/60 border border-accent/5 rounded-lg space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-display text-[9.5px] text-slate-300 uppercase tracking-wider font-semibold">
                        01. Presence Verification
                      </span>
                      {activeBoard.defect !== "Missing Component" ? (
                        <span className="inline-flex items-center gap-1 text-[9px] text-success font-mono font-bold uppercase">
                          <CheckCircle className="w-3.5 h-3.5" /> Checked
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 text-[9px] text-danger font-mono font-bold uppercase">
                          <XCircle className="w-3.5 h-3.5" /> MISSING
                        </span>
                      )}
                    </div>
                    <p className="font-sans text-[10px] text-slate-400 leading-relaxed">
                      Verifying placement logic of high-value integrated IC circuits and filter electrolytic capacitors against BOM list records.
                    </p>
                  </div>
                  ...
                  <div className="p-3 bg-[#050816]/60 border border-accent/5 rounded-lg space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-display text-[9.5px] text-slate-300 uppercase tracking-wider font-semibold">
                        02. Position Offset Verification
                      </span>
                      {activeBoard.defect !== "Misalignment" ? (
                        <span className="inline-flex items-center gap-1 text-[9px] text-success font-mono font-bold uppercase">
                          <CheckCircle className="w-3.5 h-3.5" /> Within Limits
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 text-[9px] text-danger font-mono font-bold uppercase">
                          <XCircle className="w-3.5 h-3.5" /> OFFSET ALARM
                        </span>
                      )}
                    </div>
                    <div className="flex justify-between font-mono text-[9px] text-slate-400">
                      <span>Threshold Limit: &lt; 0.15mm</span>
                      <span>Dev: {activeBoard.status === "FAIL" && activeBoard.defect === "Misalignment" ? "0.19mm" : "0.03mm"}</span>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-[#050816]/60 border border-accent/5 rounded-lg space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-display text-[9.5px] text-slate-300 uppercase tracking-wider font-semibold">
                        03. Orientation Angle (Polarity)
                      </span>
                      {activeBoard.defect !== "Polarity Mismatch" ? (
                        <span className="inline-flex items-center gap-1 text-[9px] text-success font-mono font-bold uppercase">
                          <CheckCircle className="w-3.5 h-3.5" /> Aligned
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 text-[9px] text-danger font-mono font-bold uppercase">
                          <XCircle className="w-3.5 h-3.5" /> 180° REVERSED
                        </span>
                      )}
                    </div>
                    <div className="flex justify-between font-mono text-[9px] text-slate-400">
                      <span>Allowed drift: &lt; 5.0°</span>
                      <span>Rotation: {activeBoard.status === "FAIL" && activeBoard.defect === "Polarity Mismatch" ? "180.0°" : "0.5°"}</span>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-[#050816]/60 border border-accent/5 rounded-lg space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-display text-[9.5px] text-slate-300 uppercase tracking-wider font-semibold">
                        04. Quantity Count Verification
                      </span>
                      <span className="inline-flex items-center gap-1 text-[9px] text-success font-mono font-bold uppercase">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-slate-400">
                      <div className="bg-[#050816]/80 p-1.5 rounded border border-accent/5">
                        <span className="block text-[8px] text-slate-500 uppercase">ICs detected</span>
                        <strong className="text-white">2 / 2</strong>
                      </div>
                      <div className="bg-[#050816]/80 p-1.5 rounded border border-accent/5">
                        <span className="block text-[8px] text-slate-500 uppercase">Capacitors</span>
                        <strong className="text-white">14 / 14</strong>
                      </div>
                    </div>
                  </div>
              ...
              <GlassCard className="space-y-4" hoverLift={false}>
                <div className="flex items-center gap-2 border-b border-accent/5 pb-2">
                  ...
                  <span className="font-display text-[10px] tracking-widest text-white uppercase font-bold">
                    YOLO Sensitivity Controls
                  </span>
                </div>
                <div className="space-y-3 text-left">
                  <div className="space-y-1">
                    <div className="flex justify-between font-mono text-[9px] text-slate-400">
                      <span>CONFIDENCE RATIO THRESHOLD</span>
                      <span className="text-accent font-bold">{yoloThreshold}%</span>
                    </div>
                    <input
                      type="range"
                      min="30"
                      max="95"
                      value={yoloThreshold}
                      onChange={(e) => setYoloThreshold(parseInt(e.target.value))}
